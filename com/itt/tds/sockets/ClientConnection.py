...
class ClientConnection:
    ...
    def get_connection(self):
        ...
        self.client_p_number = 9990

        if self.log:
            self.log.log_info(
                'ClientConnection', '__init__', 'opening a client socket connection.')

        self.c_socket.bind((self.h_name, self.client_p_number))
    ...
    def send_data(self):
        ...
        request = TDSRequest()  # creating TDSRequest object
        request.set_method("add-node")
        request.set_protocol_type('request')  # setting request type
        ...
        request.add_parameter("message", msg)  # setting request headers
        request.set_destination_ip(self.h_name)  # setting request headers
        request.set_destination_port(self.server_p_number)
        request.set_source_ip(self.h_name)
        request.set_source_port(self.client_p_number)
    ...
    def recv_data(self):
        ...
        response = self.seralizer_object.deserialize(
            response_bytes)
        print("Response Status : " + response.get_status())
#        print("Response Error Code : " +
#              str(response.get_error_code()))
